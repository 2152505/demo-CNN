...
class Loader(object):
    # 很有趣的一点在loader里并没有提供参数明确写法，但是趋势可以传参进来，很明显能够看出区别，不继承object对象，只拥有了__doc__ , __module__ 和 
    # 自己定义的name变量, 也就是说这个类的命名空间只有三个对象可以操作.
    # Animal类继承了object对象，拥有了好多可操作对象，这些都是类中的高级特性。
    # 对于不太了解python类的同学来说，这些高级特性基本上没用处，但是对于那些要着手写框架或者写大型项目的高手来说，这些特性就比较有用了，
    # 比如说tornado里面的异常捕获时就有用到__class__来定位类的名称，还有高度灵活传参数的时候用到__dict__来完成.
...
def get_training_data_set(num,onerow=False):
    image_loader = ImageLoader('./datasets/train-images.idx3-ubyte', num)  # 参数为文件路径和加载的样本数量
    label_loader = LabelLoader('./datasets/train-labels.idx1-ubyte', num)  # 参数为文件路径和加载的样本数量
    return image_loader.load(onerow), label_loader.load()

# 获得测试数据集。onerow表示是否将每张图片转化为行向量
def get_test_data_set(num,onerow=False):
    image_loader = ImageLoader('./datasets/t10k-images.idx3-ubyte', num)  # 参数为文件路径和加载的样本数量
    label_loader = LabelLoader('./datasets/t10k-labels.idx1-ubyte', num)  # 参数为文件路径和加载的样本数量
    return image_loader.load(onerow), label_loader.load()
